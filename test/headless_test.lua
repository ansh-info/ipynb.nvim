...
local function check(label, cond, reason)
  if cond then
    ...
    fail_test(label, reason or "assertion failed")
  end
end
...
check("inspector.lua calls execute_snippet", insp_src:find("execute_snippet") ~= nil)
check(
  "inspector.lua has no output.append monkey-patch",
  insp_src:find("output%.append%s*=") == nil,
  "found 'output.append =' in inspector.lua"
)
check(
  "inspector.lua has no vim.wait blocking call",
...
check(
  "inspector.lua has no '%-*s' format spec",
...
check("computed format string works in LuaJIT", ok_fmt, tostring(err_fmt))

-- ── 5. run_current_cell skips markdown cells ─────────────────────────────────

if ok_kernel and ok_cell then
  local orig_at_cursor = cell.cell_at_cursor
  cell.cell_at_cursor = function(_)
    return { cell_type = "markdown", start_mark = 1, end_mark = 2, index = 1 }, 1
  end
  ...
  check(
    "run_current_cell skips markdown cells",
...
check("kernel_bridge.py has _setup_ids", bridge_src:find("_setup_ids") ~= nil)
check(
  "kernel_bridge.py waits for setup reply before notifying Lua",
  bridge_src:find("_get_shell_reply%(setup_id") ~= nil,
  "_get_shell_reply(setup_id) not found - race condition fix missing"
)
check(
  "kernel_bridge.py sends status:starting AFTER setup completes",
...
if ok_insp2 then
  check(
    "highlight_inspector_buf uses no hard-coded end_col=200",
    insp_src2:find("end_col%s*=%s*200") == nil,
    "found 'end_col = 200' - will crash on short lines"
  )
  check(
    "highlight_inspector_buf clamps end_col with math.min",
